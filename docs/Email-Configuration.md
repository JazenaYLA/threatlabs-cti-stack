...
| :--- | :--- | :--- |
| `*@<BRAND_DOMAIN>` | `threatresearcher@agentmail.to` | Programmatic AI Triage (Bypasses PMG) |
| `*@<DOMAIN>` | `threatlabs@agentmail.to` | Programmatic AI Triage (Bypasses PMG) |
...
```
<MANAGEMENT_NET>/24    # Main LAN
<CTI_NET>/24  # VLAN 101 (CTI services)
```

**Command used:**

```bash
pmgsh create /config/mynetworks -cidr '<MANAGEMENT_NET>/24'
pmgsh create /config/mynetworks -cidr '<CTI_NET>/24'
```
...
```
[smtp.mail.yahoo.com]:587 jamz_yaneza@<PLACEHOLDER>:ChangeMe_YahooAppPassword
```

**Commands used:**

```bash
echo '[smtp.mail.yahoo.com]:587 jamz_yaneza@<PLACEHOLDER>:ChangeMe_YahooAppPassword' > /etc/postfix/sasl_passwd
...
```bash
pmgsh create /config/transport -domain <DOMAIN> -host <STALWART_IP> -port 25 -use_mx 0 -comment 'Deliver to Stalwart'
pmgsh create /config/transport -domain <BRAND_DOMAIN> -host <STALWART_IP> -port 25 -use_mx 0 -comment 'Deliver to Stalwart'
```
...
poll imap.gmail.com with proto IMAP
  localdomains <DOMAIN> <BRAND_DOMAIN>
  envelope "To"
...
- **`smtpaddress <DOMAIN>`**: Appends this domain if the extracted address lacks one, preventing Postfix `501 5.1.3 Bad recipient address syntax` rejections.
...
- **Web UI**: `http://<STALWART_IP>:8080` (or via Caddy: `https://mail.<DOMAIN>`)
