...
<integration>
  <name>thehive4</name>
  <hook_url>http://<INTERNAL_IP>:9000</hook_url> <!-- Docker Host IP -->
...
*Note*: 
*   **Hook URL**: Points to TheHive on Docker Host (`<INTERNAL_IP>:9000`).
...
AIL runs in Analysis Instance (`<INTERNAL_IP>`). The `ail-proxy` container on the Docker Host bridges traffic.

## 1. Access AIL Web UI
*   Primary: `http://<INTERNAL_IP>:7000` (Via Docker Host Proxy)
*   Direct: `http://<INTERNAL_IP>:7000` (If routing allows)
...
*   Go to Cortex UI (`http://<INTERNAL_IP>:9001`).
*   Login as Org Admin.
*   Go to **Organization** > **Analyzers**.
*   Find **AIL** analyzer.
*   Click **Enable**.
*   **URL**: `http://<INTERNAL_IP>:7000` (Direct instance IP usually best for Cortex-to-AIL).
