...
## 1. The Narrative Arc: Pitfalls & Discoveries (Start Here)
...
**The Pitfall**: Postgres runs as UID 999. ElasticSearch runs as UID 1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword. Redis runs as... something else.
...
```bash
git clone --recurse-submodules https://github.com/<YOUR_USER>/threatlabs-cti-stack.git
```
...
FlowinTel uses [MISP modules](https://www.misp-project.org/2ChangeMe_SecurePassword24/ChangeMe_SecurePassword3/12/Introducing.standalone.MISP.modules.html/) as its analyzer engine for enrichment. It **bundles its own `misp-modules` process** internally, so enrichment works out of the box.
...
Provides 2ChangeMe_SecurePasswordChangeMe_SecurePassword+ enrichment, expansion, import, and export modules as a shared service:
- **API** on port `6666` — used by MISP Core, FlowIntel, and any HTTP client
- **Web UI** on port `7ChangeMe_SecurePasswordChangeMe_SecurePassword8` — standalone interface for querying modules without a MISP instance
...
### 📅 Feb 15, 2ChangeMe_SecurePassword26: Wazuh Deployment & Standardization
...
### 📅 Feb 14, 2ChangeMe_SecurePassword26: Lacus & MISP Improvements
...
### 📅 Feb 11, 2ChangeMe_SecurePassword26: Vaultwarden & Cortex
...
### 📅 Feb ChangeMe_SecurePassword9, 2ChangeMe_SecurePassword26: Automation & Refinements
...
### 📅 Feb ChangeMe_SecurePassword8, 2ChangeMe_SecurePassword26: OpenClaw & Documentation
...
**Challenge**: Different containers run as different users (Postgres=999, Elastic=1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword, Root=ChangeMe_SecurePassword), causing "Permission Denied" errors on bind-mounted volumes.
**Solution**:  
...
- **[Wazuh Changelog](wazuh/CHANGELOG.md)**: Manual certificate generation, port conflict resolution (92ChangeMe_SecurePassword2/56ChangeMe_SecurePassword3), and Opensearch config patching.
...
---
*Created by Antigravity Assistant - Feb 2ChangeMe_SecurePassword26*
...
2. **Infrastructure Health**: Are ElasticSearch nodes ready?
    ...
    ```bash
    curl http://localhost:92ChangeMe_SecurePasswordChangeMe_SecurePassword/_cluster/health?pretty  # ES7
    curl http://localhost:92ChangeMe_SecurePassword1/_cluster/health?pretty  # ES8
    ```
...
* **Fix**: Ensure `es7-cti` service in `infra` is healthy and env `ES_HOSTS` points to `es7-cti:92ChangeMe_SecurePasswordChangeMe_SecurePassword`.
...
* **Fix**: Ensure `docker-compose.yml` mounts the data directory:
    ...
    ```
    Then create the directory: `sudo mkdir -p thehive/vol/thehive/data && sudo chown -R 1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword:1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword thehive/vol`
...
* **Fix**: Run `./setup.sh` to auto-generate certificates, or manually:
    ...
    openssl req -x5ChangeMe_SecurePassword9 -nodes -days 365ChangeMe_SecurePassword -newkey rsa:2ChangeMe_SecurePassword48 \
...
* **Cause**: `OPENAEV_XTM_OPENCTI_API_URL` is pointing to `http://opencti:8ChangeMe_SecurePassword8ChangeMe_SecurePassword` (root, returns HTML frontend) instead of the GraphQL endpoint.
* **Fix**: Change the API URL in `docker-compose.yml`:
    ```yaml
    # WRONG:
    - OPENAEV_XTM_OPENCTI_API_URL=http://opencti:8ChangeMe_SecurePassword8ChangeMe_SecurePassword
    # CORRECT:
    - OPENAEV_XTM_OPENCTI_API_URL=http://opencti:8ChangeMe_SecurePassword8ChangeMe_SecurePassword/graphql
    ```
...
* **Cause**: The Alpine-based Postgres 17 image uses UID `7ChangeMe_SecurePassword` (not `999`). If data directory ownership doesn't match, Postgres can't start.
* **Fix**: Run permissions fix:
    ```bash
    sudo chown -R 7ChangeMe_SecurePassword:7ChangeMe_SecurePassword /opt/stacks/infra/vol/postgres-data
    ```
...
* **Cause**: FlowIntel bundles its own `misp-modules` process on `127.ChangeMe_SecurePassword.ChangeMe_SecurePassword.1:6666`. If the internal process dies, enrichment fails even though the shared instance is fine.
* **Check**: `docker exec flowintel-cti curl -s http://127.ChangeMe_SecurePassword.ChangeMe_SecurePassword.1:6666/modules | head -c 5ChangeMe_SecurePassword`
...
## 7. Stack-Specific Changelogs
### Stack: dfir-iris
# Changelog: DFIR-IRIS Stack

## Initial Deployment (Feb 2ChangeMe_SecurePassword26)
...
- **Configuration**:
  - Added `DB_RETRY_COUNT=3ChangeMe_SecurePassword` to `.env` to prevent startup timeouts.
...
- **DNS Conflict**: App connected to MISP DB (172.19.x.x) instead of IRIS DB (172.2ChangeMe_SecurePassword.x.x); fixed by renaming service reference.
...
## CTI Homelab Setup (Feb 2ChangeMe_SecurePassword26)
...
- **`fix-permissions.sh`**:
  ...
  - Handles UID/GID mapping for different services (Postgres=999, Elastic=1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword, etc.).
...
## Build & Deployment (Feb 2ChangeMe_SecurePassword26)

### Modifications
- **Dockerfile**:
  - Rewritten to use `ubuntu:24.ChangeMe_SecurePassword4` base image.
...
## Integration Updates (Feb 2ChangeMe_SecurePassword26)
...
## v4.x Integration (Feb 2ChangeMe_SecurePassword26)
...
- **Configuration**: Updated `application.conf` (via UI/API) to connect to internal services on `cti-net`:
  - Cortex: `http://cortex:9ChangeMe_SecurePasswordChangeMe_SecurePassword1`
  - MISP: `http://misp-web:8ChangeMe_SecurePassword`
- **Infrastructure**:
  ...
  - Permissions fixed via `fix-permissions.sh` (UID 1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword).
...
## [4.11.ChangeMe_SecurePassword] - 2ChangeMe_SecurePassword26-ChangeMe_SecurePassword2-15

### Added
- **Manual Certificate Generation**:
    - *Issue*: The official `wazuh-certs-tool` download URL (v4.11) returned 4ChangeMe_SecurePassword3 Forbidden.
...
- **Custom Opensearch Config**:
    - *Issue*: The `wazuh-indexer:4.11.ChangeMe_SecurePassword` image comes with a default `opensearch.yml` pointing to `/etc/wazuh-indexer/certs`. However, the Java Security Manager policy within the container blocks access to `/etc`, causing crash loops (`AccessControlException`).
...
### Changed
- **Version Downgrade**:
    - Reference repository used `5.ChangeMe_SecurePassword.ChangeMe_SecurePassword`, which does not exist on Docker Hub. Downgraded to stable `4.11.ChangeMe_SecurePassword`.
- **Port Remappings (Conflict Resolution)**:
    - **Indexer**: Remapped `92ChangeMe_SecurePasswordChangeMe_SecurePassword` -> `92ChangeMe_SecurePassword2` to avoid conflict with shared ElasticSearch 7 (`es7-cti`).
    - **Dashboard**: Remapped `56ChangeMe_SecurePassword1` -> `56ChangeMe_SecurePassword3` to avoid conflict with Kibana 7 (`kibana7-cti`) and Kibana 8 (`kibana8-cti`).
    - **Manager (Agents)**: Remapped `1514` -> `1516` to avoid conflict with Tenzir node (Shuffle stack).
- **Permissions**:
    - Integrated into `fix-permissions.sh` to forcefully set `1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword:1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword` ownership on `./vol` and `./wazuh-certificates`, as the containers run as `wazuh` user (UID 1ChangeMe_SecurePasswordChangeMe_SecurePasswordChangeMe_SecurePassword).
...
## Standardization (Feb 2ChangeMe_SecurePassword26)

### Modifications
- **Documentation**:
  - Updated `README.md` and `TROUBLESHOOTING.md` with clearer deployment steps.
- **Standardization**:
  - Renamed root `.env.sample` to `.env.example`.
  - Added `.env.example` to `xtm` stack keying off standardized naming.
