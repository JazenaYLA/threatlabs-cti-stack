...
class AbstractManager(ABC):
    ...
    @staticmethod
    def _get_redis_connection(db=1):
        redis_host = os.environ.get('REDIS_HOST')
        redis_port = int(os.environ.get('REDIS_PORT', 6379))
        if redis_host:
             return Redis(host=redis_host, port=redis_port, db=db, decode_responses=True)
        return Redis(unix_socket_path=get_socket_path('cache'), db=db, decode_responses=True)
    ...
    def __init__(self, loglevel: int | None=None):
        ...
        self.__redis = AbstractManager._get_redis_connection()
    ...
    def is_running() -> list[tuple[str, float, set[str]]]:
        try:
            r = AbstractManager._get_redis_connection()
